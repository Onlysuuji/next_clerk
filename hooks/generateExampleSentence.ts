...
export const useGenerateExample = () => {
    ...
    const generateExample = async (word: Word) => {
        try {
            ...
            const examplePrompt = `${word.question}の単語を使って、簡単で実用的な ${showLanguage} の文を1つ生成してください。日常生活（買い物・食事・移動など）で使うシンプルな例が理想です。文章のみを出力してください。`
            ...
            // 日本語訳を取得
            const japanesePrompt = `${questionExample} を自然な日本語に翻訳してください。翻訳のみを返してください。`
